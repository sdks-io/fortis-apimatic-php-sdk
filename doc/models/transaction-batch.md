...
| `isOpen` | `?float` | Optional | 1 = Batch is_open, 0 = Batch is closed. Only 1 open batch per product_transaction_id.<br><br>**Constraints**: `>= 0`, `<= 1` | getIsOpen(): ?float | setIsOpen(?float isOpen): void |
...
| `batchCloseTs` | `?float` | Optional | Batch Close Ts in unix | getBatchCloseTs(): ?float | setBatchCloseTs(?float batchCloseTs): void |
